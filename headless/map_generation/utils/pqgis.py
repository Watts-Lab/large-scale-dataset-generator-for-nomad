...
import map_generation.utils.file as file
import map_generation.utils.geojson as geojson
...
def get_buildings_shapeFile(buildings_geojson):
    ...
    if "tags" in buildings.columns:
        tags_building = buildings["tags"].apply(
            lambda d: d.get("building") if isinstance(d, Mapping) else None
        )
        # fill missing values in base series with the tags-derived value
        building_series = building_series.fillna(tags_building)
        buildings["building"] = building_series
